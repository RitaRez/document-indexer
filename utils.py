...
def encode_to_string(word_id: int, doc_freqs: list[int, int]) -> str:

    line = "{\"word\": \"" + str(word_id) + "\", \"docs\": ["
    for doc, freq in doc_freqs:
        line += "(\"" + str(doc) + "\", \"" + str(freq) + "\"),"
    line = line[:-1]
    line += "]}\n" 
...
def download_index_shard(index: OrderedDict, index_path: str, file_name: str):
    ...
    seconds = time.time()
    with open(index_path + "indexes/" + file_name, 'wb') as fp:
        for word_id in sorted(index):

            line = encode_to_binary(word_id, index[word_id])
            # line = encode_to_string(word_id, index[word_id])
            fp.write(line)

    logging.info(f"Time to save index of shard {file_name}: {time.time() - seconds} seconds")


def index_shard(term_lexicon: dict, file_name: str, index_path: str) -> None:
    ...
    with open(index_path + "shards/" + file_name, 'r') as f:
        for line in f:
            ...
            for word, freq in word_freq.items():	
                ...
                word_id = term_lexicon[word]
                if word_id not in index:
                    index[word_id] = []
                
                index[word_id].append((doc['id'],freq))
...
def run_indexer_thread_pool(term_lexicon: dict, index_path: str, number_of_threads: int):
    """Creates a thread pool with the number of threads we want to use"""
    
    notice_files = os.listdir(index_path + "shards/")
    with Pool(number_of_threads) as pool:
        pool.starmap(index_shard, [(term_lexicon, file_name, index_path) for file_name in notice_files])
